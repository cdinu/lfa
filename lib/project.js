...
function Project(dir, config) {
  var self = this;
  self.loaded = when.try(function() {
    self.inputDir = path.resolve(dir) || process.cwd();
    self.loadedExtensions = [];
    self.targets = {};
    self.config = new Config(self, config);
    return self.config.loaded.then(function () {
      self.emit('loadExtensions', self);
      self.bootstrapExtensions();
      self.emit('loadedExtensions', self);

      self.emit('loadTarget', self);
      var Target = self.targets[self.config.target];
      if (typeof(Target) !== 'function') {
        throw new Error('No such target: \"' + self.config.target + '\"');
      }
...
Project.prototype.bootstrapExtensions = function() {
  var self = this;
  _.each(self.config.parsedExtensions, function(Extension) {
